      ...
      <div className="glass p-12 text-center max-w-md w-full">
        <div className="relative w-20 h-20 mx-auto mb-8">
          ...
          <div className="relative w-20 h-20 rounded-full flex items-center justify-center text-3xl"
            style={{background:'linear-gradient(135deg, #10b981, #3b82f6)', boxShadow:'0 0 40px rgba(16,185,129,0.4)'}}>✓</div>
        </div>
        ...
        <div className="glass p-10">
          <div className="mb-8">
            <Link href="/dashboard" className="text-sm font-semibold flex items-center gap-2 mb-8 transition-colors hover:text-primary"
              style={{color:'var(--fg-muted)'}}>← Back to Dashboard</Link>
            ...
            <p className="text-sm" style={{color:'var(--fg-muted)'}}>Update your account security credentials.</p>
          </div>
          {error && (<div className="mb-6 p-4 rounded-xl text-sm font-medium"
          ...
          <form onSubmit={handleChange} className="space-y-5">
            <div>
              ...
              <input type="password" className="input-field" placeholder="Your current password" value={currentPassword}
                ...
                <div className="mt-2 flex gap-1">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-1 flex-1 rounded-full transition-all duration-300" style={{
                      background: newPassword.length > i * 3 ? (newPassword.length < 6 ? '#f59e0b' : newPassword.length < 10 ? '#3b82f6' : '#10b981') : 'var(--glass-border)'
                    }} />
                  ))}
                </div>
              )}
            ...
            <button type="submit" disabled={loading} className="btn btn-primary w-full py-4 text-base mt-2">
              {loading ? (<span className="flex items-center gap-2">
                <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> Updating...</span>
              ) : 'Update Password →'}
            </button>
          </form>
        </div>
