      ...
      <div className="w-full max-w-md relative">
        <div className="glass p-10">
          <div className="mb-8">
            ...
            <p className="text-sm" style={{color:'var(--fg-muted)'}}>Choose a strong password for your account.</p>
          </div>
                ...
                <div className="mt-2 flex gap-1">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-1 flex-1 rounded-full transition-all" style={{
                      background: password.length > i * 3 ? (password.length < 6 ? '#f59e0b' : password.length < 10 ? '#3b82f6' : '#10b981') : 'var(--glass-border)'
                    }} />
                  ))}
                </div>
              )}
          ...
          <p className="text-center text-sm mt-6" style={{color:'var(--fg-muted)'}}>
            Remember it? <Link href="/auth/login" className="font-bold hover:text-primary transition-colors" style={{color:'var(--primary)'}}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
